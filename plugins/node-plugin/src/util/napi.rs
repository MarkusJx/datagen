...
use napi::sys::{napi_env, napi_value, setup};
...
unsafe fn run_napi_inner<F: for<'a> FnOnce(Env, JsObject)>(f: F) -> i32 {
    ...
    unsafe extern "C" fn napi_reg_func<F: for<'a> FnOnce(Env, JsObject)>(
        env: napi_env,
        exports: napi_value,
    ) -> napi_value {
        ONCE.call_once(|| {
        ...
        let napi_exports = JsObject::from_raw(env, exports).unwrap();
        ...
        module_init_fn(napi_env, napi_exports);
...
pub fn run_napi<F: for<'a> FnOnce(Env, JsObject) -> anyhow::Result<R>, R: Debug>(
    func: F,
) -> anyhow::Result<R> {
    let _guard = GUARD.lock().unwrap();
    if ONCE.is_completed() {
        return Err(anyhow!("run_napi can only be called once"));
    }

    let (sender, receiver) = channel::<anyhow::Result<R>>();
    let code = unsafe {
        run_napi_inner(move |env, exports| {
            let res = func(env, exports).map_err(|e| anyhow!(e));
